        ...
        match device.device_descriptor() {
            Ok(device_desc) => {
                match device.config_descriptor(0u8) {
                    Ok(cfg) => println!("config_descriptor: {:?}", cfg),
...
pub fn setup_mode(k: &mut Device, mode: &str) {
    ...
    match k.open() {
        Ok(mut c) => {
            let cfg = c.active_configuration();
            let index: u16 = cfg.unwrap() as u16;
            println!("usb active configuration index: {:?}", index);

            match c.claim_interface(0u8) {
                Ok(_) => println!("interface claimed"),
                Err(_) => println!("failed to claim interface"),
            }
            ...
            match ctrl_buff {
                Some(b) => {
                    ...
                    match c.write_control(33u8, 9u8, 768u16, index, b, Duration::from_millis(300)) {
                        Ok(_siz) => println!("Ok"),
    ...
    for r in 0..colors.len() {
        ...
        match r_wrote {
            Ok(d_siz) => {
                ...
                match handle.write_bulk(/*endpoint*/ 1u8, &msg, Duration::from_millis(500)) {
